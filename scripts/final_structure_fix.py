...
def main():
    ...
    <div style="margin-top: 40px; padding: 25px; border-left: 5px solid #4a674a; background: #fdfdfd; border-radius: 0 10px 10px 0;">
        <h4 style="margin-top: 0; color: #2c3e50; font-size: 1.1em;">(Key Takeaway)</h4>
        <p style="font-weight: bold; color: #333; line-height: 1.6; margin-bottom: 0;">"외로움을 나약함의 증거가 아닌 뇌의 '생존 신호'로 재정의하고, 통증이 느껴질 때마다 의도적인 논리적 사유를 가동하여 기만적 시스템으로부터 전두엽의 주권을 사수하십시오."</p>
    </div>

    <div style="background: #f4f7f4; padding: 30px; border-radius: 12px; color: #333; border: 1px solid #D4E2D4; margin-top: 30px;">
    ...
    print("Final Structure Remaster (Key Takeaway above Reflection)...")
    ...
    if response.status_code == 200:
        print(f"Success! Final Structured Link: {response.json().get('link')}")
    else:
